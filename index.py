...
class App:
    def __init__(self, window, file_path, sort_path):
        ...
        self.exts = [
            ['.mov', '.mp4', '.ogg', '.mp3'],
            ['.png', '.jpg', '.jpeg', '.gif'],
            ['.txt', '.docx', '.doc', '.pdf', 'xlsx'],
            ['.deb', '.AppImage'],
            ['.py', '.c', '.cpp', '.js', '.css', '.html', '.json', 'cs', '.vhd']
        ]
        ...
        self.folder_label = Label(
            master=window, borderwidth=2, relief='sunken', textvariable=self.folder_path)
        ...
        self.mkdir_button = Button(
            text='Make directories', command=self.make_dirs)
        ...
        if file_path.is_file() or file_path.is_dir():
            for ls in self.exts:
                for ex in ls:
                    if ex == ext:
                        shutil.copy(
                            self.sort_path + file, self.sort_path + self.dirs[self.exts.index(ls)])
                        os.remove(self.sort_path + file)

        # other extension
        if file_path.is_file():
            if ext != '':
                shutil.copy(self.sort_path + file,
                            self.sort_path + self.dirs[5])
                os.remove(self.sort_path + file)
